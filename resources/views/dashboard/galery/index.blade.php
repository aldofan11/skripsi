...
<div class="card mb-4">
  ...
  <div class="card-body">
    <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns" id="table_data">
      @include('dashboard.galery.pagination')
    </div>
  </div>
</div>
...
<script>
  ...
  $("#tambah").click(function() {
    $("#exampleModalLabel").text('Tambah Galeri');
    $("#submit").text('Tambah');
  ...
  const editData = (id) => {
    ...
    $("#inputID").val(id)
    formType = "UPDATE";
    $("#exampleModalLabel").text('Update Galeri');
    $("#submit").text('Update');
    ...
    axios.get(URL_NOW + `/${id}`)
      .then(({
        ...
        $("#preview").html(`<img src="${BASE_URL}@getPath(galeries)${photo}" alt="gambar ${title}" class="img-fluid" width="300">`)
  ...
  $("#form").on('submit', async (e) => {
    ...
    if (formType === "CREATE") {
      await new Promise((resolve, reject) => {
        axios.post(`{{ route('galeri.store') }}`, FormDataVar, {
            headers: {
              'contentType': false,
              'processData': false
            }
          ...
          .then(({
            ...
            swal({
              icon: 'success',
              title: 'Berhasil',
              text: 'Galery Berhasil Ditambahkan'
            });
          })
          .catch(err => {
            ...
            axios.delete(`${URL_NOW}/${id}`)
              ...
              .catch(err => {
                ...
                swal({
                  icon: 'error',
                  title: 'Gagal',
                  text: 'Galeri gagal berhasil dihapus'
                })
              })
          })
        }

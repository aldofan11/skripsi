...
<div class="card mb-4">
  ...
  <div class="card-body">
    <div class="dataTable-wrapper dataTable-loading no-footer sortable searchable fixed-columns" id="table_data">
      @include('dashboard.document.pagination')
    </div>
  </div>
</div>
...
<script>
  ...
  $("#tambah").click(function() {
    $("#exampleModalLabel").text('Tambah Informasi Pembuatan Dokumen');
    $("#submit").text('Tambah');
  ...
  const editData = (id) => {
    ...
    $("#inputID").val(id)
    formType = "UPDATE";
    $("#exampleModalLabel").text('Update Informasi Pembuatan Dokumen');
    $("#submit").text('Update');
  ...
  $("#form").on('submit', async (e) => {
    ...
    if (formType === "CREATE") {
      await new Promise((resolve, reject) => {
        axios.post(`{{ route('dokumen.store') }}`, FormDataVar, {
            headers: {
              'contentType': false,
              'processData': false
            }
          ...
          .then(({
            ...
            swal({
              icon: 'success',
              title: 'Berhasil',
              text: 'Informasi Pembuatan Dokumen Berhasil Ditambahkan'
            });
          })
          .catch(err => {
        ...
        axios.post(`${URL_NOW}/${id}`, FormDataVar, {
          ...
          .then(({
            ...
            swal({
              icon: 'success',
              title: 'Berhasil',
              text: 'Informasi Pembuatan Dokumen Berhasil Diupdate'
            });
            refresh_table(URL_NOW)
            // data.message.body
          })
            ...
            axios.delete(`${URL_NOW}/${id}`)
              .then(({
                ...
                swal({
                  icon: 'success',
                  title: 'Berhasil',
                  text: 'Informasi Pembuatan Dokumen berhasil dihapus'
                })
                refresh_table(URL_NOW)
              })
              .catch(err => {
                ...
                swal({
                  icon: 'error',
                  title: 'Gagal',
                  text: 'Informasi Pembuatan Dokumen gagal berhasil dihapus'
                })
              })
          })
        }
